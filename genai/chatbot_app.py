"""
🌍 Climate Intelligence Command Center v4 (FINAL)
Fully aligned with 80-city global producer + create_wh.sql schema.
...
.chat-msg-user {
    ...
    box-shadow: 0 4px 16px #6366f133; animation: slideUp 0.3s ease-out;
}
.chat-msg-bot {
...
def load_gold_data():
    ...
    try:
        ...
        for i, (_, r) in enumerate(df.iterrows()):
            # Use visibility_km if available, fall back to visibility_miles
            vis_val = r.get('visibility_km', None)
            vis_unit = "km"
            if vis_val is None or vis_val == 0:
                vis_val = r.get('visibility_miles', 'N/A')
                vis_unit = "miles"
            ...
            doc = (
                ...
                f"Precipitation: {r.get('precipitation_mm','N/A')} mm. "
...
def build_sql_answer(question, gc):
    # Schema info aligned with create_wh.sql + producer
    schema_info = """
    ...
    2. dim_location
       Columns: location_key, city, country, continent, state, latitude, longitude, region
       (80 cities across 6 continents)
    ...
    4. dim_weather_type
       ...
       (condition values: Clear, Clouds, Rain, Snow, Thunderstorm, Mist, Fog, Haze, etc.)

    RULES:
    - JOIN fact with dim_location using location_key for city/country/continent queries
    ...
    - Return ONLY a valid PostgreSQL SELECT. No markdown, no explanations.
    """
    r = gc.chat.completions.create(
        model="llama-3.1-8b-instant",
        messages=[
            {"role": "system", "content": f"You are a PostgreSQL expert. Output ONLY the SQL query.\n{schema_info}"},
    ...
    try:
        ...
        if not results:
            return "No results found. Try rephrasing your question.", "sql"

        r2 = gc.chat.completions.create(
            model="llama-3.1-8b-instant",
            messages=[
                {"role": "system", "content": "Summarize weather query results clearly with specific numbers. Be concise (2-4 sentences)."},
...
def build_rag_answer(question, coll, gc, mode="rag"):
    ...
    prompts = {
        "rag": (
            "You are an expert climate analyst monitoring 80 cities across 6 continents. "
            "Answer ONLY from the provided data. Be specific with numbers, cities, and countries. "
            "If data doesn't contain the answer, say so clearly."
        ),
        "report": (
            "You are a global weather intelligence analyst monitoring 80 cities across 6 continents. "
            "Generate a structured report:\n"
            "📋 **SUMMARY** — 1-2 sentence overview\n"
            "🔍 **KEY FINDINGS** — Top 3-4 findings with specific numbers\n"
            "⚠️ **RISKS & ALERTS** — Any extreme weather or anomalies\n"
            "🌍 **REGIONAL HIGHLIGHTS** — Notable conditions by continent\n"
            "✅ **RECOMMENDATIONS** — 2-3 actionable items"
        ),
        "anomaly": (
            "You are a senior meteorologist analyzing global anomalies:\n"
            "🔴 **WHAT'S UNUSUAL** — Identify anomalies with specific numbers and cities\n"
...
def try_direct_answer(question, gold_df):
    """Answer simple min/max questions directly from DataFrame."""
    q = question.lower()
    cities = gold_df.drop_duplicates(subset=['city'], keep='last')

    # Helper to format city name with country
    def fmt(row):
        country = row.get('country', '')
        return f"{row['city']}, {country}" if country else row['city']
    ...
    if any(k in q for k in ["hottest", "highest temp", "warmest", "highest temperature"]):
        ...
        lines = "\n".join([f"  {i+1}. **{fmt(r)}** — {round(r['temperature_fahrenheit'],1)}°F"
    ...
    if any(k in q for k in ["most humid", "highest humidity", "humid"]):
        ...
        return (f"💧 **{fmt(row)}** has the highest humidity at **{round(row['humidity_percent'],1)}%** "
                f"({round(row['temperature_fahrenheit'],1)}°F)."), "rag"

    if any(k in q for k in ["windiest", "most windy", "highest wind", "strongest wind"]):
        row = cities.loc[cities['wind_speed_mph'].idxmax()]
        return (f"💨 **{fmt(row)}** has the highest wind speed at **{round(row['wind_speed_mph'],1)} mph** "
                f"({round(row['temperature_fahrenheit'],1)}°F)."), "rag"
    ...
    if any(k in q for k in ["extreme weather", "extreme events", "how many extreme"]):
        ...
        city_list = ", ".join([f"**{fmt(r)}** ({round(r['temperature_fahrenheit'],1)}°F)" for _, r in ext.iterrows()])
        return (f"⚠️ **{count} extreme weather event(s)** detected:\n\n{city_list}\n\n"
                f"These cities have conditions outside normal ranges."), "rag"

    if any(k in q for k in ["heatwave", "heat wave"]):
        if 'is_heatwave' in cities.columns:
            hw = cities[cities['is_heatwave'] == 1]
        else:
            hw = cities[cities['temperature_fahrenheit'] > 100]
        if len(hw) == 0:
            return "✅ No heatwave conditions detected.", "rag"
        city_list = ", ".join([f"**{fmt(r)}** ({round(r['temperature_fahrenheit'],1)}°F)" for _, r in hw.iterrows()])
        return f"🔥 **{len(hw)} city/cities** with heatwave conditions (>100°F):\n\n{city_list}", "rag"

    if any(k in q for k in ["extreme cold", "freezing cold", "below zero"]):
        if 'is_extreme_cold' in cities.columns:
            ec = cities[cities['is_extreme_cold'] == 1]
        else:
            ec = cities[cities['temperature_fahrenheit'] < 10]
        if len(ec) == 0:
            return "✅ No extreme cold conditions detected.", "rag"
        city_list = ", ".join([f"**{fmt(r)}** ({round(r['temperature_fahrenheit'],1)}°F)" for _, r in ec.iterrows()])
        return f"🥶 **{len(ec)} city/cities** with extreme cold (<10°F):\n\n{city_list}", "rag"
...
def render_app():
    ...
    <div class="top-bar">
        ...
        <span class="top-status">
            ...
            {city_count} CITIES &nbsp;│&nbsp; {continent_count} CONTINENTS &nbsp;│&nbsp;
        ...
        for idx, (_, city) in enumerate(row_chunk.iterrows()):
            with cols[idx]:
                ...
                <div class="city-card {cls}">
                    <div class="city-name">{nm}{flag}{badge}</div>
    ...
    if "msgs" not in st.session_state:
        st.session_state.msgs = [{
            ...
            "text": (
                ...
                f"{city_count} cities** across **{continent_count} continents** in real-time.\n\n"
                "I support 4 intelligence modes:\n"
                "• **RAG** — Ask anything about current weather data\n"
                "• **SQL** — Quantitative queries (averages, counts, rankings, by continent/region)\n"
                "• **Report** — Structured global weather intelligence reports\n"
